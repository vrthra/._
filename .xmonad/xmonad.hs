...
myKeys conf@(XConfig {XMonad.modMask = modMask}) = M.fromList $
    ...
    -- launch a terminal
    --[ ((modMask .|. controlMask, xK_x), spawn $ XMonad.terminal conf)
    [ ((modMask, xK_x), spawn $ XMonad.terminal conf)

    -- launch dmenu
    , ((modMask, xK_z), spawn $ "exe=`cat ~/.dmenu|dmenu -nb black -nf white -p : -b -sb black -sf white` && eval \"exec $exe\"")

    -- clipboard
    , ((modMask, xK_c), spawn $ "~/.home/bin/xcut")
...
main = do
    ...
    xmonad $ defaultConfig {
        ...
        logHook = dynamicLogWithPP (xmobarPP {
                  ...
                  , ppCurrent = xmobarColor "red" "" . shorten 5
                  , ppHidden = xmobarColor "blue" "" . shorten 5
                  , ppTitle = xmobarColor "red" "" . shorten 150
                  } ) >> updatePointer (Relative 0.5 0.5)
      } `additionalKeys`
      [
       ((0, xK_Pause), spawn "xtrlock"),
       ((0, xK_Caps_Lock), spawn "touchmouse")
      ]
...
{-- main = do
    xmproc <- spawnPipe "/home/rn151090/.cabal/bin/xmobar"
    xmonad $ defaultConfig {
      -- simple stuff
        terminal           = myTerminal,
